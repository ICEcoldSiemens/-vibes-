...
// Class for EDM playlist
public class EDM implements MusicGenre {
    ...
    // Constructor to loop through each song in EDM playlist + submenu to run playlist or quit.
    public EDM() {
        // Stores music file paths in a Files array
        try {
            files[0] = new File("music_file/edm/Besomorph & Anthony Keyrouz - Death Bed (ft. Lunis).wav");
            files[1] = new File("music_file/edm/Coopex & Yohan Gerber - Radioactive (ft. LUNIS).wav");
            files[2] = new File("music_file/edm/Coopex - You Broke Me First (ft. Britt Lari).wav");
            files[3] = new File("music_file/edm/EMDI x Britt Lari - We Are Perfect.wav");
            files[4] = new File("music_file/edm/Robbe, New Beat Order, Britt Lari - Kings & Queens.wav");
